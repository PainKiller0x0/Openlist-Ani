...
<main>
  ...
  <section class="card">
    ...
    <label for="rssUrl">添加 RSS 链接</label>
    <div class="top-actions"><input id="rssUrl" type="url" placeholder="https://mikanani.me/RSS/Subscription/..." autocomplete="off"><button id="rssBtn" onclick="addRss()">添加订阅</button></div>
...
<script>
  ...
  async function refresh(){try{const s=await api('/api/ui/state');renderStatus(s);$('targetPath').textContent=s.download_path||'由配置决定';$('rssList').innerHTML=s.rss_urls.length?s.rss_urls.map(u=>`<div class="feed"><div class="feed-url"><span class="pill">运行中</span>${esc(u)}</div><button class="danger" onclick="removeRss(${JSON.stringify(u)})">删除</button></div>`).join(''):'<div class="muted">还没有订阅。添加 RSS 后，这里会显示“运行中”。</div>';$('taskList').innerHTML=s.tasks.length?s.tasks.map(t=>`<li><strong>${esc(t.title)}</strong><span class="pill">${esc(t.state)}</span><div class="muted small">${esc(t.anime_name||'')} ${t.episode?`· 第 ${esc(t.episode)} 集`:''}</div></li>`).join(''):'<li class="muted">暂无进行中的任务</li>'}catch(e){setStatus('rssStatus','状态读取失败：'+e.message,true)}}
  ...
  async function addRss(){const url=$('rssUrl').value.trim();if(!url){setStatus('rssStatus','请先填写 RSS 链接',true);return}const b=$('rssBtn');b.disabled=true;setStatus('rssStatus','正在验证 RSS…');try{const d=await api('/api/ui/rss',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url})});setStatus('rssStatus',d.message+(d.preview?`\n最新条目：${d.preview}`:''));$('rssUrl').value='';await refresh()}catch(e){setStatus('rssStatus',e.message,true)}finally{b.disabled=false}}
  async function removeRss(url){if(!confirm('删除这个订阅？删除后不会再自动检查它。'))return;try{const d=await api('/api/ui/rss',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({url})});setStatus('rssStatus',d.message);await refresh()}catch(e){setStatus('rssStatus',e.message,true)}}
