...
export async function renderPreview(ctx) {
  ...
  const refresh = async (button) => {
    ...
    ctx.flow.episodeOffset = Number(value('#previewEpisodeOffset') || 0);
    ctx.flow.excludePatterns = value('#previewExclude');
    setLoading(button, true, '⌛ 识别中');
    try { ctx.flow.preview = await api.post('/api/ui/rss/preview', {url: ctx.flow.rssUrl, name: ctx.flow.preferredName, anime_name: ctx.flow.animeName, download_directory_name: ctx.flow.directoryName, episode_offset: ctx.flow.episodeOffset, exclude_patterns: ctx.flow.excludePatterns}); await renderPreview(ctx); }
    catch (error) { toast(error.message, 'error'); setLoading(button, false); }
  };
  ['#previewName', '#previewAnimeName', '#previewDirectoryName', '#previewEpisodeOffset', '#previewExclude'].forEach((selector) => document.querySelector(selector).addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); refresh(document.querySelector('#refreshPreview')); } }));
  ...
  document.querySelector('#refreshMetadata')?.addEventListener('click', async (event) => {
    ...
    try {
      ...
      ctx.flow.preview = await api.post('/api/ui/rss/preview', {url: ctx.flow.rssUrl, name: ctx.flow.preferredName || value('#previewName'), anime_name: value('#previewAnimeName'), download_directory_name: value('#previewDirectoryName'), episode_offset: Number(value('#previewEpisodeOffset') || 0), exclude_patterns: value('#previewExclude')});
  ...
  document.querySelector('#confirmPreview').addEventListener('click', async (event) => {
    ...
    try {
      ...
      const result = await api.post('/api/ui/rss', {url: ctx.flow.rssUrl, name: value('#previewName'), anime_name: value('#previewAnimeName'), download_directory_name: value('#previewDirectoryName'), episode_offset: Number(value('#previewEpisodeOffset') || 0), exclude_patterns: value('#previewExclude'), tmdb_id: data.tmdb_id || null, season: Number(firstIncluded?.season || 1), confirmed: true});
