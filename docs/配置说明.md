...
| `interval_time` | int | `300` | RSS 抓取间隔（秒），允许 60～86400；内置设置页修改后立即生效 |
...
默认格式：`{anime_name} - S{season:02d}E{episode:02d}`
...
内置网页的“设置”中可以直接修改 OpenList 地址、下载根目录和重命名规则。保存 OpenList 地址前会验证连接、登录凭据和离线下载工具；如果同时修改下载根目录，还会使用新地址列目录验证。任一验证失败时整次设置保存会被拒绝，不会写入新配置。OpenList 地址、下载根目录和重命名规则修改会立即用于新建任务，已经创建的任务仍使用原地址和目录。
